...
- React ベースの Web UI
- データ圧縮によるディスク容量削減（miniSEED zstd / スクリーンショット WebP / 地震区間抽出）
- 地震検出時・JMA 照合時の Slack 通知（info チャンネル、任意）
- 統計ダッシュボード（日別検出数・MaxCount 分布・JMA 照合率・検出感度を Chart.js で可視化）
...
# データ圧縮（miniSEED zstd / スクリーンショット WebP）
uv run rsudp-compress       # -n で dry-run

# miniSEED から地震前後の区間のみ抽出（obspy 使用、不可逆）
uv run rsudp-compress --extract-quake
...
└── rsudp/
    ...
    │   └── compress.py             # データ圧縮（rsudp-compress）
    ├── compress.py                 # 圧縮ロジック（miniSEED zstd / スクリーンショット WebP）
    ...
    └── webui/
        └── api/
            ...
            ├── statistics.py       # 統計・検出感度の集計ロジック
            └── schemas.py          # API リクエスト/レスポンスのスキーマ
...
- `min_magnitude`: 最小マグニチュードフィルタ（`earthquake_only=true` 時のみ有効、`/api/screenshot/` と `/api/screenshot/statistics/` で対応）
...
data:
    ...
    miniseed: data/data # miniSEED 波形アーカイブ（zstd 圧縮対象、任意）
    miniseed_extract_quake: true # 地震前後の区間のみ残す抽出を有効化（不可逆、任意）

webapp:
    static_dir_path: frontend/dist # React ビルド出力パス

# 観測局の位置（任意）。設定すると統計タブの検出感度分析が有効になる
station:
    latitude: 35.681
    longitude: 139.767

# Slack 通知（任意）。info チャンネルへ地震検出・JMA 照合を通知する
slack:
    from: rsudp
    bot_token: xoxb-xxxxxxxx
    info: # 検出・照合通知先（レート制限なし）
        channel:
            name: rsudp-quake
            id: C0XXXXXXX
    error: # ヘルスチェック失敗通知先（interval_min でレート制限）
        channel:
            name: rsudp-error
            id: C0YYYYYYY
        interval_min: 10
...
`station` は緯度・経度の両方が揃っている場合のみ有効。`slack.info` を設定すると地震検出通知（機能）と JMA 照合通知が info チャンネルへ送られる（未設定時は通知しない）。
...
- **ミラーリポジトリ**: GitHub (`github.com/kimata/rsudp`)
