...
class TestQuakeDatabaseInit:
    ...
    def test_init_creates_tables(self, temp_dir):
        ...
        config = {"data": {"quake": str(temp_dir / "test.db")}}
        QuakeDatabase(config)

        with sqlite3.connect(temp_dir / "test.db") as conn:
...
class TestInsertEarthquake:
    ...
    def test_insert_new_earthquake(self, temp_dir, sample_earthquake_jst):
    ...
    def test_insert_duplicate_returns_false(self, temp_dir, sample_earthquake_jst):
    ...
    def test_insert_updates_existing(self, temp_dir, sample_earthquake_jst):
        """既存の地震データが更新されることを確認."""
        config = {"data": {"quake": str(temp_dir / "test.db")}}
        db = QuakeDatabase(config)
...
class TestGetEarthquakeForTimestamp:
    ...
    def test_find_earthquake_exact_time(self, temp_dir, sample_earthquake_jst):
        """正確な時刻で地震を検索できることを確認."""
        config = {"data": {"quake": str(temp_dir / "test.db")}}
        db = QuakeDatabase(config)
        db.insert_earthquake(**sample_earthquake_jst)
    ...
    def test_find_earthquake_within_window(self, temp_dir, sample_earthquake_jst):
        """時間窓内で地震を検索できることを確認."""
        config = {"data": {"quake": str(temp_dir / "test.db")}}
        db = QuakeDatabase(config)
        db.insert_earthquake(**sample_earthquake_jst)
    ...
    def test_no_match_outside_window(self, temp_dir, sample_earthquake_jst):
...
class TestGetAllEarthquakes:
    ...
    def test_get_all_earthquakes_empty(self, temp_dir):
    ...
    def test_get_all_earthquakes_ordered(self, temp_dir):
    ...
    def test_get_all_earthquakes_limit(self, temp_dir):
...
class TestGetEarthquakeTimeRanges:
    ...
    def test_get_earthquake_time_ranges(self, temp_dir, sample_earthquake_jst):
...
class TestCountEarthquakes:
    ...
    def test_count_earthquakes_empty(self, temp_dir):
    ...
    def test_count_earthquakes(self, temp_dir, sample_earthquake_jst):
        """正しい件数が返されることを確認."""
        config = {"data": {"quake": str(temp_dir / "test.db")}}
        db = QuakeDatabase(config)
        db.insert_earthquake(**sample_earthquake_jst)
