...
- 🔔 **Slack 通知** - 地震検出時、および気象庁地震情報と照合が取れた際の自動通知
- 📈 **統計ダッシュボード** - 日別検出数・MaxCount 分布・JMA 照合率・検出感度（震央距離×MaxCount）を Chart.js で可視化
...
- **フィルタ機能**:
    ...
    - **日付フィルタ**: 年/月/日による階層的絞り込み（JST 基準）
    - **リアルタイム統計**: 全体とフィルタ後のスクリーンショット数を表示
- **統計タブ**: 「一覧 / 統計」タブ切り替えで、日別検出数・MaxCount 分布・JMA 照合率の推移・検出感度（震央距離×MaxCount）を Chart.js で表示
- **表示機能**:
...
- `days`: 集計対象の遡り日数（`/api/statistics/daily`・`/api/statistics/association`、デフォルト 90）

`/api/statistics/sensitivity` は `config.yaml` の `station`（観測局座標）が設定されている場合のみ点データを返します（未設定時は `station: null`）。
...
SHAKE-2025-08-15-104524.webp  # 圧縮後は WebP 形式（後述の圧縮機能）
...
- **quake.db**: 気象庁地震データ
    - event_id: 地震イベントID
    - detected_at: 発生時刻（UTC、表示時に JST へ変換）
...
data ディレクトリの肥大化を抑えるため、データを圧縮して保存します。

| 対象                                     | 形式       | 方式                        | 削減率の目安 |
| ---------------------------------------- | ---------- | --------------------------- | ------------ |
| miniSEED 波形（`data/data`）             | `*.zst`    | zstd                        | 約 55%       |
| miniSEED 波形（地震区間抽出・任意）      | `*.eq.zst` | obspy で地震前後のみ抽出    | 約 97%       |
| スクリーンショット（`data/screenshots`） | `*.webp`   | cwebp lossy q95 + sharp_yuv | 約 70%       |

- 継続的な圧縮はバックグラウンド監視が 1 日 1 回自動実行します。
- 既存データの一括移行や手動実行には `rsudp-compress` を使用します。

```bash
uv run rsudp-compress                  # 圧縮を実行
uv run rsudp-compress -n               # dry-run（対象と削減見込みを表示）
uv run rsudp-compress --screenshots-only
uv run rsudp-compress --miniseed-only
uv run rsudp-compress --extract-quake  # 地震前後の区間のみ残して miniSEED を間引く（不可逆）
```

- miniSEED 波形は Web UI から参照されないため zstd で透過的に圧縮保持します（当日のファイルは書き込み中のため対象外）。圧縮ファイルは `zstd -d` または `rsudp.compress.decompress_miniseed()` で展開できます。
- さらに `--extract-quake` を指定すると、**地震前後の区間のみを残して miniSEED を間引きます**（不可逆）。前1分・後ろは規模可変（`M<5`:10分 / `5≤M<6.5`:20分 / `M≥6.5`:30分）で、地震に該当しない日のファイルは削除します。地震は後からクロールで追加されるため、確定待ちマージン（既定2日）より新しいファイルは対象外です。抽出済みは `*.eq.zst` となり再処理されません。地震解析に必要な波形は保全しつつ miniSEED を大幅に削減できます（実測で約16G→0.4G）。
- スクリーンショットは WebP へ変換し、メタデータ（STA/LTA 等）は cache.db が保持します。
- 圧縮には `zstd` と `cwebp`（Ubuntu の `webp` パッケージ）が必要です（Docker イメージには同梱済み）。
- miniSEED 圧縮を有効にするには config.yaml に `data.miniseed`（例: `data/data`）を設定します。
- 地震区間抽出（不可逆）を自動実行するには `data.miniseed_extract_quake: true` を設定します（既定は無効）。
...
- **更新頻度**: デフォルト30秒間隔でファイルに UNIX タイムスタンプを書き込み（一時ファイル + rename でアトミックに更新）
- **異常検知**: 監視対象の異常停止時にはライブネスファイルの更新を停止
- **自動クリーンアップ**: アプリケーション正常終了時にライブネスファイルを自動削除

この機能により、外部監視システム（`rsudp-healthz`）がファイルに記録されたタイムスタンプを確認することで、rsudpの稼働状況を監視できます。
