...
Usage:
  rsudp-compress [-c CONFIG] [-n] [-D] [--miniseed-only | --screenshots-only]

Options:
  -c CONFIG               : 設定ファイルを指定します。[default: config.yaml]
  -n                      : dry-run モード。実際には圧縮せず、対象を表示します。
  -D                      : デバッグモードで動作します。
  --miniseed-only         : miniSEED の圧縮のみ実行します。
...
def _log_result(label: str, result: rsudp.compress.CompressResult, *, dry_run: bool) -> None:
    ...
    if dry_run:
        ...
        logging.info(
            "%s%s: 処理 %d件, スキップ %d件, 削減 %.1f MB (%.1f -> %.1f MB)",
            prefix,
            label,
            result.processed,
            result.skipped,
    ...
    *,
    ...
    if not screenshots_only:
        if config.data.miniseed is None:
            logging.warning("data.miniseed が設定されていないため miniSEED 圧縮をスキップします")
        else:
